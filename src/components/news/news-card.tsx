...
export default function NewsCard({ news, onNotInterested }: NewsCardProps) {
  ...
  const handleReadFull = (e: React.MouseEvent) => {
    e.stopPropagation()
    window.open(news.url, "_blank") // Opens the full news in a new tab
  }
  ...
  const handleNotInterested = () => {
    toast.warning("We will show you less content like this.")
    if (onNotInterested) {
      onNotInterested(news._id) // ✅ Ensure _id is used as a string
    }
  }
  ...
  return (
    ...
    >
      ...
      <CardContent className="p-4">
        <h3 className="text-xl font-semibold mb-2 line-clamp-2">{news.title}</h3>
        <p className="text-gray-500 text-sm mb-4 line-clamp-3">{news.summary}</p>
        <p className="text-xs text-gray-500">
          {news.publishedAt ? new Date(news.publishedAt).toLocaleDateString() : "Unknown date"}
        </p>
      </CardContent>
      ...
      <CardFooter className="px-4 pb-4 pt-0 flex flex-col gap-2">
        <div className="flex justify-between w-full">
          <div className="flex gap-2">
            <Button variant="outline" size="sm" onClick={(e) => { e.stopPropagation(); handleInterested() }}>
              <ThumbsUp className="h-4 w-4 mr-1" />
              Interested
            </Button>
            <Button variant="outline" size="sm" onClick={(e) => { e.stopPropagation(); handleNotInterested() }}>
              <ThumbsDown className="h-4 w-4 mr-1" />
              Not Interested
            </Button>
          </div>
